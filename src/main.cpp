...
#if defined(NDEBUG)
# error "Generationchanger cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9d945f700fd83610d013299f7775bdbf0d7acd77b8c9879874218783c720aec6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Generationchanger: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Generationchanger Signed Message:\n";
...
{
    // Generationchanger: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 44000 blocks
    nSubsidy >>= (nHeight / 44000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Generationchanger: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Generationchanger: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1507306515, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "1507305564";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0439ef0e675c0210b5cf2e551fd61f006d5f1e8189b44b1d1eeac53ce876a30e9a4a155f7755b93b418fbbcb0e38a27493b442cba64b098335c733e6aa2e43e73e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1507306515;
        ...
        assert(block.hashMerkleRoot == uint256("0x05e2fe2ff5846a627db5f058ae1639ce4f5413d3687809698fb35e51d56ae0fe"));
...
unsigned char pchMessageStart[4] = { 0x94, 0xf4, 0xa2, 0xd4 }; // Generationchanger: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GenerationchangerMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GenerationchangerMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GenerationchangerMiner(CWallet *pwallet)
{
    printf("GenerationchangerMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("generationchanger-miner");
    ...
    try { loop {
        ...
        printf("Running GenerationchangerMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GenerationchangerMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GenerationchangerMiner, pwallet));
}
